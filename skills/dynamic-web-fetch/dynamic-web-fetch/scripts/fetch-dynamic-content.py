...
async function fetchContent() {
    ...
    try {
        ...
        if (extractModeArg === 'html') {
            ...
            // Пока возвращаем HTML, преобразование будет в Python
            content = await page.content();
        } else {
...
'''
    ...
    try:
        # Запускаем Node.js скрипт с аргументами и полным контролем окружения
        env = {}
        # Только основные переменные, которые нам нужны
        ...
        result_process = subprocess.run(
            ['/usr/bin/node', '-e', node_script, url, wait_for_arg, selector_type_arg, custom_script_arg, extract_mode_arg],
            capture_output=True,
            text=True,
